...
def count_elements_before_cycle(ll: 'a linked list object') -> int:
    cycle_begin_item = ll['last']['next']

    count = 0
    curr = ll
    while curr is not cycle_begin_item:
        count += 1
        curr = curr['next']
...
if __name__ == '__main__':
    ...
    ll['last']['next'] = traverse_to_index(7, ll)
    print(ll['last']['next'])

    print(count_elements_before_cycle(ll))
